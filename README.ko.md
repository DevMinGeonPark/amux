...
`install.sh`는 바이너리를 복사한 뒤(동의하면) 셸 rc에 관리 블록을 추가해
`~/.local/bin`을 `PATH`에 넣고 탭 완성을 켭니다. 이어서 **SSH 로그인 시 picker
자동 실행**(원격/헤드리스 서버에 유용) 여부도 묻습니다 — `y`로 켜면 됩니다.
재실행해도 블록이 중복되지 않고 제자리에서 갱신되니 안전합니다.

또는 `bin/amux`를 `PATH` 어딘가에 두기만 해도 됩니다(셸 연동 생략).
